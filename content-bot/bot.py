...
async def is_semantically_similar(new_title: str) -> bool:
    ...
    try:
        ...
        for vector in existing_vectors:
            ...
            if similarity > 0.90:
                logger.info(f"Takroriy kontent topildi. O'xshashlik: {similarity:.2f}")
                return True
        return False
    except Exception as e:
...
async def fetch_and_process_feeds(context: ContextTypes.DEFAULT_TYPE, manual_run: bool = False):
    ...
    for category, url in RSS_FEEDS.items():
        ...
        for entry in feed.entries[:5]:
            if not await is_post_exists(entry.link):
                all_new_entries.append(entry)
    
    if not all_new_entries:
        if manual_run: await context.bot.send_message(chat_id=ADMIN_USER_ID, text="Qidiruv yakunlandi. Yangi maqolalar topilmadi.")
        return
    ...
    if await is_semantically_similar(latest_entry.title):
        if manual_run: await context.bot.send_message(chat_id=ADMIN_USER_ID, text=f"Yangi maqola topildi, lekin u mazmunan takroriy: {latest_entry.title}")
        return

    try:
        if manual_run: await context.bot.send_message(chat_id=ADMIN_USER_ID, text=f"Yangi maqola topildi: {latest_entry.title}\nQayta ishlanmoqda...")
        
        # --- YAKUNIY, BATAFSIL KONTENT UCHUN PROMPT ---
        prompt = f"""
        ...
        image_url = extract_image_from_html(latest_entry)

        post_data = {
            'title': post_json.get('title'),
            'summary': post_json.get('summary'),
            'content': post_json.get('content_markdown'),
            'category': post_json.get('category'),
            'imageUrl': image_url,
            'original_link': latest_entry.link,
        ...
        if image_url:
            await context.bot.send_photo(chat_id=TELEGRAM_CHANNEL_ID, photo=image_url, caption=final_telegram_message, parse_mode=ParseMode.MARKDOWN)
        else:
        ...
        logger.info(f"Yangi post qo'shildi: {post_json.get('title')}")
